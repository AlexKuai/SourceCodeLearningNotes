...
{
    // 处理异常并将 ErrorContext 传递给下一个异常过滤器
...
internal class DeveloperExceptionPageMiddlewareImpl
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        catch (Exception ex)
        {
            ...
            try
            {
                ...
                // 如果是 BadHttpRequestException 异常
                if (ex is BadHttpRequestException badHttpRequestException)
                {
                    // 利用 BadHttpRequestException.StatusCode 设置 HTTP 响应状态码（400 - 499）
    ...
    private Task DisplayException(ErrorContext errorContext)
    {
        ...
        // 如果 HTTP 请求报头的 Accept 不包含 text/html 媒体类型，则以纯文本内容返回异常信息
        ...
        // 如果 HTTP 请求报头的 Accept 包含 text/html 媒体类型
...
internal sealed class ExceptionHandlerMiddlewareImpl
{
    ...
    public Task Invoke(HttpContext context)
    {
        ...
        static async Task Awaited(ExceptionHandlerMiddlewareImpl middleware, HttpContext context, Task task)
        {
            ...
            {
                // 利用 ExceptionDispatchInfo 捕获异步发生的异常
                edi = ExceptionDispatchInfo.Capture(exception);
            }
    ...
    private async Task HandleException(HttpContext context, ExceptionDispatchInfo edi)
    {
        ...
        try
        {
            ...
            // 清除 HTTP 响应缓冲区和匹配到的 Endpoint
            ClearHttpContext(context);

            // 以 IExceptionHandlerFeature 接口形式将 ExceptionHandlerFeature 添加到 HttpContext 的特性集合中
            // 以 IExceptionHandlerPathFeature 接口形式将 ExceptionHandlerFeature 添加到 HttpContext 的特性集合中
            // 提供给后续异常处理管道的中间件
            context.Features.Set<IExceptionHandlerFeature>(exceptionHandlerFeature);
            context.Features.Set<IExceptionHandlerPathFeature>(exceptionHandlerFeature);
            // 设置 HTTP 响应状态码为 500
            context.Response.StatusCode = DefaultStatusCode;
            // 在异常处理器中间件中只要发生异常就需要清除 HTTP 响应中的缓存报头
            ...
            // 如果 IExceptionHandler 集合中的异常处理器都没有表示处理了异常
            if (!handled)
            {
                // 如果 ExceptionHandlerOptions.ExceptionHandler 属性配置的异常处理器存在
                if (_options.ExceptionHandler is not null)
                {
                    // 利用 ExceptionHandlerOptions.ExceptionHandler 属性配置的异常处理器处理异常
        ...
        finally
        {
            ...
            // 因为原始的 IServiceProvider 是由 IServiceScope 创建的，需要通过 HttpResponse.RegisterForDisposeAsync 方法释放
            if (oldScope != null)
            {
    ...
    private static IApplicationBuilder SetExceptionHandlerMiddleware(IApplicationBuilder app, IOptions<ExceptionHandlerOptions>? options)
    {
        ...
        if (app.Properties.TryGetValue(RerouteHelper.GlobalRouteBuilderKey, out var routeBuilder) && routeBuilder is not null)
        {
            ...
            return app.Use(next =>
            {
                ...
                // 所以需要重新构建一条以 EndpointRoutingMiddleware 中间件作为管道头部处理器并连接后续中间件的请求处理管道用于处理异常
...
public class StatusCodePagesMiddleware
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        // 检查 IStatusCodePagesFeature 特性是否标记跳过响应状态码页中间件
...
public static class StatusCodePagesExtensions
{
    ...
    // 注册响应状态码页中间件
    // 提供用于格式化响应内容的字符串，可以包含一个 "{0}" 占位符，用于格式化字符串时替换为具体的 HTTP 响应状态码
    // 以及一个媒体类型字符串
    ...
    // 注册响应状态码页中间件
    // 提供 Action<IApplicationBuilder> 配置，用于注册中间件并构建请求处理器管道
        ...
        string? queryFormat = null)
    {
        ...
        if (app.Properties.TryGetValue(RerouteHelper.GlobalRouteBuilderKey, out var routeBuilder) && routeBuilder is not null)
        {
            ...
            return app.Use(next =>
            {
                // 在 Minimal API 编程模型中，EndpointRoutingMiddleware 中间件的注册可能早于 StatusCodePagesMiddleware 中间件
                // 所以需要重新构建一条以 EndpointRoutingMiddleware 中间件作为管道头部处理器并连接后续中间件的请求处理管道用于处理状态码
                var newNext = RerouteHelper.Reroute(app, routeBuilder, next);
