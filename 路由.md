...
internal sealed class RouteEndpointDataSource : EndpointDataSource
{
    ...
    //    这属于默认绑定策略
    //    注意：
    //    默认利用查询字符串作为绑定源有个前提，就是路由系统终结点的路由模式中不存在同名（忽略大小写）的路由模式参数部分
    //    如果存在只能通过显式绑定 FromQueryAttribute 特性的方式指定绑定源
...
internal sealed partial class EndpointRoutingMiddleware
{
    ...
    private Task<Matcher> InitializeCoreAsync()
    {
        ...
        try
        {
            ...
            // 注意：
            // 不直接创建 DfaMatcher 的目的是需要利用 CompositeEndpointDataSource 监控保存的所有 EndPointDataSource 的变化
...
public static class EndpointRouteBuilderExtensions
{
    ...
    // 注意：
    // 返回值类型都是 IEndpointConventionBuilder，可以用来注册对 EndpointBuilder 的约定配置
