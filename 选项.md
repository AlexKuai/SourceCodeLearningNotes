        ...
        where TOptions : class
{
    ...
    // 每个文件配置源都可以利用 IFileProvider.Watch 方式所得到的 IChangeToken 来监控文件，并在文件发生变动的情况下发出回调
    ...
    // 注册监听配置源发生变动后的回调
    // 可以注册多个回调（委托链）
    // 使用 ChangeTrackerDisposable 封装 Action 回调，便于外部保存 IDisposable 并可以通过调用 Dispose 方法从委托链冲取消回调注册
...
public class ConfigureNamedOptions<TOptions> : IConfigureNamedOptions<TOptions> where TOptions : class
{
    ...
    public virtual void Configure(string? name, TOptions options)
    {
        ...
        // 使用配置的原则是传递的命名需要与当前配置的 Name 名称相同，或者配置的 Name 为 null（成为通用配置）
...
public class ValidateOptions<TOptions> : IValidateOptions<TOptions> where TOptions : class
{
    ...
    public ValidateOptionsResult Validate(string? name, TOptions options)
    {
        ...
        // 返回没有匹配到验证配置时的 ValidateOptionsResult
...
public static class OptionsServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddOptions(this IServiceCollection services)
    {
        ...
        // 此处应该注册为 Transient 生命周期，因为目前选项缓存只在 OptionsMonitor<> 构造函数中注入
        ...
        Type configureType)
    {
        ...
        {
            // 服务注册生命周期为 Transient
            // 由于 FindConfigurationServices 返回的不是泛型类型定义，所以此处注册的 configureType 不能是泛型开放类型
...
public class OptionsBuilder<TOptions> where TOptions : class
{
    ...
    {
        ThrowHelper.ThrowIfNull(services);
        // 可以通过调用 Services 返回 IServiceCollection 继续使用对应的扩展方法注册服务
        Services = services;
        // name 为 null 则使用空字符串命名
        // 所以通过 OptionsBuilder 提供的 Api 注册的选项配置不存在为 null 的通用配置
