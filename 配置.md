...
public abstract class ConfigurationProvider : IConfigurationProvider
{
    ...
    // 如果是文件配置源，则会利用它持有的 IFileProvider 对象，并调用 Watch 方法返回的 IChangeToken 注册 OnReload 方法，并在配置源发生改变后调用
...
public abstract class FileConfigurationSource : IConfigurationSource
{
    ...
    // 就会通过调用 GetFileProvider 扩展方法，使用应用程序根目录创建 IFileProvider
    ...
    // 如果 Path 是相对路径，则最终会通过调用 EnsureDefaults 方法创建默认的 IFileProvider 并作为共享数据，并存储在 IConfigurationBuilder.Properties 上
    public void ResolveFileProvider()
    {
        ...
        {
            // 得到目录路径（如果结尾没有 "\" 就当作文件，返回路径中最后的 "\" 表示的目录路径）
            ...
            if (Directory.Exists(directory))
            {
                ...
                // 设置可能包含相对路径的文件名（包含后缀名）
                // 实际用作 IFileProvider 的 subPath
                Path = pathToFile;
            }
        }
    }
...
public static class ConfigurationExtensions
{
    ...
    // 判断配置节是否存在（创建的 IConfigurationSection 可能根本不存在）
...
{
    // 重写加载
    // 内部实际调用 IConfigurationProvider.Load 方法重新加载配置字典
    void Reload();
...
public interface IConfigurationSection : IConfiguration
{
    ...
    // 如果是叶配置节，则返回配置字典中以 Path 为键的对应的值
...
public static class ConfigurationBinder
{
    ...
    private static void BindInstance(Type type, BindingPoint bindingPoint, IConfiguration config, BinderOptions options)
    {
        ...
        if (config != null && config.GetChildren().Any())
        {
            ...
            if (bindingPoint.Value is null)
            {
                ...
                // 1. 实现 IDictionary<,> 的字典对象（未通过 TypeIsADictionaryInterface 判断）
                // 2. 实现 ICollection<> 的集合对象（部署数组类型，并且未通过 IsArrayCompatibleInterface 判断）
                // 3. 非集合实例对象
                // 作为非集合实例绑定时，满足以下情况，创建实例将直接通过构造函数的参数绑定配置
                // 1. 唯一的构造函数存在参数
    ...
    private static object? BindSet(Type type, IEnumerable? source, IConfiguration config, BinderOptions options)
    {
        ...
        // HashSet 集合内部是调用元素实例的 GetHashCode 方法得到散列值来确定位置的
