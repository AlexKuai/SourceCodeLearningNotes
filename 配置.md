...
public class ConfigurationBuilder : IConfigurationBuilder
{
    ...
    // 在通过 IConfigurationSource 配置源构建对应的 IConfigurationProvider 配置提供者的过程中提供一组共享的数据帮助构建
    ...
    // 根据不同的 IConfigurationSource 配置源得到对应 IConfigurationProvider 配置提供者，最终利用配置提供者集合构建 IConfigurationRoot 配置根
...
public abstract class ConfigurationProvider : IConfigurationProvider
{
    ...
    // 配置字典
    // 配置键是以 “:” 分段的字符串
    // 字典中每一条记录的配置键都表示从根出发到叶配置节的字符串（绝对路径）
    ...
    // 得到父路径下的所有子路径配置键
    // earlierKeys 代表通过累加器从其他 IConfigurationProvider 中收集配置键
    ...
    // 配置源发生改变后，激活使用 GetReloadToken 方法返回的 IChangeToken 注册的回调
    // 文件配置源会利用 IFileProvider.Watch 方法返回的 IChangeToken 注册此方法，并在配置源发生改变后调用
...
public abstract class FileConfigurationSource : IConfigurationSource
{
    ...
    // 根据绝对路径构建 IFileProvider
    // 如果 Path 是相对路径，在构建 IConfigurationProvider 时会通过调用 EnsureDefaults 方法创建默认 IFileProvider 并作为共享数据
...
public static class FileConfigurationExtensions
{
    ...
    // 使用应用程序根目录创建 IFileProvider 设置共享数据
...
public static class ConfigurationExtensions
{
    ...
    // true：从当前配置节之后枚举（使用相对路径）
    // false: 从当前配置节（包含）枚举（使用绝对路径）
...
public static class ConfigurationBinder
{
    ...
    // 根据给定的配置节、子路径、type 表示的目标类型得到配置值
    ...
    {
        // 如果目标类型实现 IConfigurationSection，则直接返回
        ...
        if (config != null && config.GetChildren().Any())
        {
            ...
            // 如果是接口类型必须是 IEnumerable<>、ICollection<>、IReadOnlyCollection<>、IReadOnlyList<> 类型，并且是被构建的泛型类型
            // 所有数组类型在 JIT 编译期的程序集加载和方法表创建时，CLR 都会为相应的数组类型实现一些集合相关的接口
            // 比如 T 类型的数组（T 不是值类型），那么 T[] 最终会实现 
            // IEnumerable<T>、ICollection<T>、IReadOnlyCollection<T>、IReadOnlyList<T> 以及相应的非泛型接口
            ...
            // 如果 type 表示的目标类型不是集合并且通过 Get 相关方法调用（不是 Bind 方法）
            if (bindingPoint.Value is null)
            {
                ...
                // 作为普通实例绑定，并将绑定完成的对象作为绑定点新的值
                // 满足以下情况时，创建实例将直接通过构造函数的参数绑定配置
                // 1. 唯一的构造函数存在参数
                // 2. 得到类型定义中所有只读属性、属性的 Set 方法是继承自父类的、属性的 Set 方法是非虚实例的，这时候
                //    构造函数参数列表的参数名称必须在得到的属性列表中全部存在（忽略大小写）
    ...
    private static Array BindArray(Type type, IEnumerable? source, IConfiguration config, BinderOptions options)
    {
        ...
        {
            // 否则从泛型接口的第一个泛型实参得到，比如 IEnumerable<>、ICollection<T>、IReadOnlyCollection<T>、IReadOnlyList<T> 得到类型 T
        ...
        foreach (IConfigurationSection section in config.GetChildren())
        {
            ...
            try
            {
                ...
                BindInstance(
                    ...
                    config: section,
                    options: options);
                // 将得到的绑定实例与已存在元素合并，添加到末尾
                // 如果没有绑定成功数组中不会存在为 null 的元素
    ...
    private static object? BindSet(Type type, IEnumerable? source, IConfiguration config, BinderOptions options)
    {
        ...
        // 仅支持泛型参数为 string 和枚举类型作为 key
        // HashSet 集合内部是调用元素实例的 GetHashCode 方法得到的值来确定位置的
    ...
    private static void BindConcreteDictionary(object? dictionary, Type dictionaryType, IConfiguration config, BinderOptions options)
    {
        ...
        foreach (IConfigurationSection child in config.GetChildren())
        {
            try
            {
                ...
                // 创建绑定点的初始值提供者
                // 由于 BindInstance 方法内部会在绑定非集合时调用 BindingPoint.Value 方法检查是否为空
                // 这时候字典内部就一定会存在字典的键为 Key 的记录，并且字典的值可能为 null
                // 这也就是为什么在绑定字典时，即使字典的值绑定不成功也会存在值为 null 的记录
    ...
    private static void BindProperties(object instance, IConfiguration configuration, BinderOptions options)
    {
        ...
        if (options.ErrorOnUnknownConfiguration)
        {
            ...
            // 不满足超集原则，则抛出 InvalidOperationException
    ...
    private static void BindProperty(PropertyInfo property, object instance, IConfiguration config, BinderOptions options)
    {
        ...
        // 创建绑定点的初始值提供者
        // 使用创建实例时属性的默认值作为绑定点初始值
        // 如果时只读属性或属性的 Set 方法非公开并且不绑定非公开属性，则设置绑定点只读
