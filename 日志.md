...
public class ConsoleLoggerProvider : ILoggerProvider, ISupportExternalScope
{
    ...
    // 用来保存注册 IOptionsMonitor<>.OnChange 回调后的注册对象（实现 IDisposable）
    // 可以通过调用 IDisposable.Dispose 取消注册回调
...
#pragma warning disable CS0618
            logFormatter = options.Format switch
            {
                ...
                // 缓存中没有指定名称的格式化器，则使用名称为 simple 的格式化器
                _ => _formatters[ConsoleFormatterNames.Simple],
            };
            if (options.FormatterName == null)
            {
                // 用 ConsoleLoggerOptions 选项中的配置去更新名称为 systemd 或 simple 的内建格式化器的 ConsoleFormatterOptions 选项中的配置
...
#pragma warning restore CS0618
        ...
        // 用新得到的 ConsoleFormatter 格式化器和更新后的 ConsoleLoggerOptions 选项更新每个创建的 ILogger
    ...
    private static void UpdateFormatterOptions(ConsoleFormatter formatter, ConsoleLoggerOptions deprecatedFromOptions)
    {
        if (formatter is SimpleConsoleFormatter defaultFormatter)
        {
            ...
            {
                // SimpleConsoleFormatterOptions 可以控制颜色行为和单行显示行为
...
internal sealed class ConsoleLogger : ILogger
{
    ...
    // 由 ConsoleLoggerProvider 在调用 ConsoleLoggerProvider.SetScopeProvider 方法时传入
    // 实现日志范围的活动跟踪
    ...
    public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
    {
        ...
        // 格式化器对日志荷载进行格式化并通过 StringWriter 写入缓冲区
...
public abstract class ConsoleFormatter
{
    ...
    // 大致的具体步骤如下：
    // 1. 利用 LogEntry<TState> 中的 TState、Exception 通过 Func<TState, Exception> 委托格式化日志消息
    // 2. 根据 LoggerFilterOptions.CaptureScopes 判断是否需要捕获日志范围
    //    根据 ConsoleFormatterOptions.IncludeScopes 判断是否需要在输出中包含范围信息
    // 3. 利用 Scope 中的 TState 以及 TextWriter 通过 IExternalScopeProvider.ForEachScope<> 方法将范围消息写入 TextWriter 缓冲区
    //    或者利用 Activity 中 W3C 格式的范围消息，根据 ActivityTrackingOptions 枚举值创建 ActivityLogScope 作为 TState
...
public static class ConsoleLoggerExtensions
{
    ...
    public static ILoggingBuilder AddConsole(this ILoggingBuilder builder)
    {
        ...
        // 注册 ConsoleLoggerOptions 选项配置依赖这两个服务注册
        builder.AddConfiguration();
        ...
        // 本质是利用 IConfiguration 将配置绑定到选项上
