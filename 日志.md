...
public static class LoggerProviderOptions
{
    public static void RegisterProviderOptions<TOptions, TProvider>(IServiceCollection services) where TOptions : class
    {
        ...
        // 由于来源于文件配置，所以会使用 IOptionsMonitor<> 作为依赖注入时的服务类型，可以通过注册 IOptionsChangeTokenSource<> 来监控文件的变动
        // 并在变动后发出回调更新选项
...
```C#
// 提供各种基于日志等级名称的日志记录方法
// 在运行时调用 ILogger.Log<> 方法时将表示日志的内容载荷 TState 泛型实参指定为 FormattedLogValues
