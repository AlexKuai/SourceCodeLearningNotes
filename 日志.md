...
public static class LoggerProviderOptions
{
    public static void RegisterProviderOptions<TOptions, TProvider>(IServiceCollection services) where TOptions : class
    {
        ...
        // 由于配置信息来源于文件，会使用 IOptionsMonitor<> 作为依赖注入时的服务类型，可以通过注册 IOptionsChangeTokenSource<> 来监控文件的变动
        // 并在变动后发出回调更新选项
...
public class ConsoleLoggerProvider : ILoggerProvider, ISupportExternalScope
{
    ...
    private void SetFormatters(IEnumerable<ConsoleFormatter>? formatters = null)
    {
        ...
        if (!added)
        {
            // ConsoleFormatterNames.Simple 对应 simple 字符串
...
#pragma warning restore CS0618
        ...
        // 将新得到的 ConsoleFormatter 格式化器和更新后的 ConsoleLoggerOptions 选项传给每个创建的 ILogger
...
internal sealed class ConsoleLogger : ILogger
{
    ...
    public bool IsEnabled(LogLevel logLevel)
    {
        // 日志时间等级只要不为 LogLevel.None 即为 true
