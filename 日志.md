...
internal sealed class Logger : ILogger
{
    ...
    // 数组的元素数量与注册的 ILoggerProvider 数量不一致
    // 如果注册的 ILoggerProvider 没有一个实现了 ISupportExternalScope 接口，则数量一致
    // 如果注册的 ILoggerProvider 至少有一个实现了 ISupportExternalScope 接口，则数量少于注册的 ILoggerProvider 数量
    // 因为实现了 ISupportExternalScope 接口的所有 ILoggerProvider 本质上共用一个 IExternalScopeProvider 提供日志范围的活动跟踪
    ...
    public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
    {
        ...
        // 调用由 ILoggerProvider 创建的 ILogger 对所关联的日志源发送日志事件
    ...
    // 可以利用此方法检测是否存在至少一个可以通过最低日志事件等级或过滤规则的 ILogger
    // 即可以将日志事件发送到至少一个 ILoggerProvider 创建的 ILogger
    public bool IsEnabled(LogLevel logLevel)
    {
        ...
        for (; i < loggers.Length; i++)
        {
            ...
            // 通过后需要继续利用 ILoggerProvider 创建的 ILogger 检查是否满足最低日志事件等级或其他过滤规则
...
internal readonly struct LoggerInformation
{
    ...
    // 实现了 ISupportExternalScope 接口就表明需要借助 IExternalScopeProvider 提供日志范围的活动跟踪
...
{
    // 通过调用 ILoggingBuilder.AddConfiguration 方法配置 providerName、categoryName、logLevel
    // 通过调用 ILoggingBuilder.AddFilter 方法配置 providerName、categoryName、filter
...
public interface ILoggerFactory : IDisposable
{
    ...
    // 创建的 ILogger 只是提供应用程序写入日志事件的入口作用
...
public class LoggerFactory : ILoggerFactory
{
    ...
    private (MessageLogger[] MessageLoggers, ScopeLogger[]? ScopeLoggers) ApplyFilters(LoggerInformation[] loggers)
    {
        ...
        // 如果有表明存在实现了 ISupportExternalScope 接口的 IServiceProvider 需要通过 IExternalScopeProvider 提供日志范围的活动跟踪
