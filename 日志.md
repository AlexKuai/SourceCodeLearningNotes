...
internal sealed class Logger : ILogger
{
    ...
    public bool IsEnabled(LogLevel logLevel)
    {
        ...
        {
            ref readonly MessageLogger loggerInfo = ref loggers[i];
            // 当前 MessageLogger 不满足最低日志事件等级则继续下一个检查
            if (!loggerInfo.IsEnabled(logLevel))
            {
                continue;
            }
            ...
            // 只要找到一个满足最低日志事件等级或过滤规则的 ILogger 就直接退出循环
    ...
    // 开始日志范围的活动跟踪
    // 本质上就是由 ILoggerProvider 创建一个表示日志范围的结构
    ...
    public IDisposable? BeginScope<TState>(TState state) where TState : notnull
    {
        ...
        // 此处的 Scope 作用是将不同 ILoggerProvider 创建的日志范围作为 IDisposable 对象收集起来
        // 因为其自身实现了 IDisposable 便于对这些日志范围统一出栈
...
```C#
// 日志信息
// 每个 ILoggerProvider 和一个日志类别对应一个 LoggerInformation
...
internal readonly struct MessageLogger
{
    ...
    // 通过 Category 和 ProviderTypeFullName 根据最优筛选逻辑得到匹配的 LoggerFilterRule 中的 LogLevel 属性值
    public LogLevel? MinLevel { get; }

    // 通过 Category 和 ProviderTypeFullName 根据最优筛选逻辑得到匹配的 LoggerFilterRule 中的 Filter 属性值
    public Func<string?, string?, LogLevel, bool>? Filter { get; }

    // 根据最低日志等级和过滤规则方法方法过滤发送的日志事件
    public bool IsEnabled(LogLevel level)
    {
        // 日志事件等级小于最低日志级别的直接返回 false
...
public class LoggerFactory : ILoggerFactory
{
    ...
    private (MessageLogger[] MessageLoggers, ScopeLogger[]? ScopeLoggers) ApplyFilters(LoggerInformation[] loggers)
    {
        ...
        foreach (LoggerInformation loggerInformation in loggers)
        {
            ...
            // 匹配规则：
            // 1. 具体的针对 ILoggerProvider 名称（类型全名或别名）的规则优先级高于默认（Default）规则
            // 2. 日志类别基于前缀最多匹配规则（对于 Foo.Bar.Baz 的日志类别，配置中 Foo.Bar 的规则比 Foo 的规则优先级更高）
            // 3. 有多个优先级相同的规则选择最后一个
...
internal sealed class LoggerFactoryScopeProvider : IExternalScopeProvider
{
    ...
    public void ForEachScope<TState>(Action<object?, TState> callback, TState state)
    {
        ...
        // 如果使用 Activity.Start 方法开始日志范围，则 _currentScope.Value 等于 null
        // 因为开始日志范围没有通过调用 ILogger.BeginScope<> 方法
        // Report 方法将直接返回
...
public partial class Activity : IDisposable
{
    ...
    // 由于 ExecutionContext 和 IAsyncLocalValueMap 都是不可变的
    ...
    // 设置 ParentId
    // 在分布式应用中，通过从请求头中取出 W3C 格式的父操作 Id 赋值给当前 Activity
    // 实现分布式链路跟踪
...
public static class LoggingServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddLogging(this IServiceCollection services, Action<ILoggingBuilder> configure)
    {
        ...
        // 注册选项核心服务
        // 日志系统中很多实现类型需要通过依赖注入的方式注入 IOptions<>、IOptionsSnapshot<>、IOptionsMonitor<> 这些选项服务
        services.AddOptions();
    ...
    private void LoadDefaultConfigValues(LoggerFilterOptions options)
    {
        ...
        foreach (IConfigurationSection configurationSection in _configuration.GetChildren())
        {
            ...
            else
            {
                ...
                if (logLevelSection != null) // if (logLevelSection.Exists())
                {
...
public static class LoggingBuilderExtensions
{
    ...
    // 此处 IConfiguration 为 LogLevel 配置节的父配置节，父配置节一般命名为 Logging
...
{
    // 根据不同的 ILoggerProvider 类型名称，遍历 LoggingConfiguration，从保存的 IConfiguration 中得到对应名称的 IConfigurationSection
    // 然后通过 IConfigurationBuilder.AddConfiguration 扩展方法将 IConfigurationSection 构建为 ChainedConfigurationSource
...
public class ConsoleLoggerProvider : ILoggerProvider, ISupportExternalScope
{
    ...
    // 默认为 NullExternalScopeProvider 
    // 表示需要通过 ISupportExternalScope.SetScopeProvider 方法设置 IExternalScopeProvider，否则不支持收集日志范围
...
#pragma warning disable CS0618
            ...
            if (options.FormatterName == null)
            {
                ...
                // 那么 FormatterOptions 属性就没有通过选项配置的 ConsoleFormatterOptions
                // 所以需要在这里依靠通过选项配置的 ConsoleLoggerOptions 覆盖填充配置
...
internal sealed class ConsoleLogger : ILogger
{
    ...
    // 因为 ConsoleLoggerProvider 实现了 IExternalScopeProvider 接口，所以最终则会由 IExternalScopeProvider 统一负责日志范围的活动跟踪（压栈）
    // 所以不需要通过 ConsoleLogger 开启日志范围的活动跟踪（这个方法不会被调用）
...
{
    // 注册控制台日志核心服务
    // 并注册基于文件配置源的 ConsoleLoggerOptions 选项配置
    ...
    // 并注册基于文件配置源的 ConsoleFormatterOptions 选项配置
        ...
        where TFormatter : ConsoleFormatter
    {
        ...
        // 所以可以通过 IOptionsMonitor<> 在配置改变后发出回调更新选项
        ...
        where TFormatter : ConsoleFormatter
    {
        ...
        // 会先调用上面的 AddConsoleFormatter 方法，注册基于文件配置源的 ConsoleFormatterOptions 选项配置
