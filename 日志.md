...
internal sealed class Logger : ILogger
{
    ...
    // 因为实现了 ISupportExternalScope 接口的 ILoggerProvider 共用一个 IExternalScopeProvider 提供日志范围的活动跟踪
    ...
    // 可以利用此方法检查是否存在至少一个可以通过最低日志事件等级或过滤规则的 ILoggerProvider
    // 即至少可以将日志事件发送到至少一个 ILoggerProvider 创建的 ILogger
