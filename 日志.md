...
public class LoggerFactory : ILoggerFactory
{
    ...
    // 创建一个独立的 DisposingLoggerFactory（实现了 ILoggerFactory）
...
public abstract class ConsoleFormatter
{
    ...
    // 3. 利用 Scope.State 作为消息载荷通过 IExternalScopeProvider.ForEachScope<> 方法将范围消息写入 TextWriter 缓冲区
    //    或者利用 Activity 的 W3C 格式的范围消息，根据 ActivityTrackingOptions 枚举值创建 ActivityLogScope 作为消息载荷
