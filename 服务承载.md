...
public abstract class BackgroundService : IHostedService, IDisposable
{
    ...
    {
        // 利用外部的 CancellationToken，创建一个基于链接的 CancellationTokenSource
        ...
        // 基于链接的 CancellationTokenSource 的作用是既可以通过被链接的 CancellationToken 触发回调
        // 也可以通过基于链接的 CancellationTokenSource 创建的 CancellationToken 触发回调
        _stoppingCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);

        // 创建一个长时间运行的任务
        // ExecuteAsync 方法由具体实现类重写
        // 此处传入的 CancellationToken 是基于链接的 CancellationTokenSource 创建的
        // 用于停止这个长时间运行的任务
        _executeTask = ExecuteAsync(_stoppingCts.Token);

        // 如果任务同步完成，直接返回这个已经完成的任务
    ...
    public virtual async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            // 停止服务就是通过基于链接的 CancellationTokenSource 调用 Cancel 方法来发出通知，取消任务
            _stoppingCts!.Cancel();
        }
        finally
        {
            // 异步函数完成分 3 种情况
            // 1. 任务成功完成
            // 2. 通过 _stoppingCts!.Cancel() 取消任务（任务非成功完成）
            // 3. 通过传入的 CancellationToken 取消等待
    ...
    // 释放操作
    // 通过调用 CancellationTokenSource.Cancel 方法来发出通知，停止任务
...
public partial class HostBuilder : IHostBuilder
{
    ...
    // 应用配置
    // 通过收集的针对应用的配置构建
    // 宿主配置在构建应用配置时会以 ChainedConfigurationSource 的形式作为第一个添加的配置源
    ...
    // 表示 IHostEnvironment.ContentRootPath 目录的 IFileProvider
    // 一般为应用程序根目录 AppContext.BaseDirectory
    ...
    public IHostBuilder UseServiceProviderFactory<TContainerBuilder>(IServiceProviderFactory<TContainerBuilder> factory) where TContainerBuilder : notnull
    {
        ...
        // 利用 ServiceFactoryAdapter 适配器封装 IServiceProviderFactory<TContainerBuilder>
    ...
    public IHostBuilder UseServiceProviderFactory<TContainerBuilder>(Func<HostBuilderContext, IServiceProviderFactory<TContainerBuilder>> factory) where TContainerBuilder : notnull
    {
        ...
        // 利用 ServiceFactoryAdapter 适配器封装 Func<HostBuilderContext, IServiceProviderFactory<TContainerBuilder>>
    ...
    public IHostBuilder ConfigureContainer<TContainerBuilder>(Action<HostBuilderContext, TContainerBuilder> configureDelegate)
    {
        ...
        // 利用 IConfigureContainerAdapter 适配器封装针对 TContainerBuilder 的配置
    ...
    internal static (HostingEnvironment, PhysicalFileProvider) CreateHostingEnvironment(IConfiguration hostConfiguration)
    {
        var hostingEnvironment = new HostingEnvironment()
        {
            ...
            // 存在以下三种情况：
            // 1. HostDefaults.ContentRootKey 表示的配置节不存在，则使用 AppContext.BaseDirectory 表示的应用程序根目录
            // 2. HostDefaults.ContentRootKey 表示的配置节的值为绝对路径，则使用 HostDefaults.ContentRootKey 配置节的值
            // 3. HostDefaults.ContentRootKey 表示的配置节的值为相对路径，则使用 AppContext.BaseDirectory 表示的应用程序根目录与 HostDefaults.ContentRootKey 配置节的值拼接
    ...
    {
        // 利用 IHostEnvironment.ContentRootPath 创建 IFileProvider 
        // 保存在共享数据字典中，为后续所有的文件配置源提供共享的 IFileProvider
        // 并且将宿主配置作为链接数据源，合并到应用配置
...
#pragma warning restore CS0618 
        ...
        // 注册 IHost，使用 Func<IServiceProvider, IHost> 工厂
    ...
    {
        // 非以下系统环境情况下注册 IHostLifetime，使用 ConsoleLifetime 类型
...
// 宿主构建扩展方法
// 提供额外添加配置的方法
// 大部分的配置不接收 HostBuilderContext 参数
public static class HostingHostBuilderExtensions
{
    ...
    // 使用 DefaultServiceProviderFactory 作为 IServiceProvider 工厂
    ...
    internal static void ApplyDefaultAppConfiguration(HostBuilderContext hostingContext, IConfigurationBuilder appConfigBuilder, string[]? args)
    {
        ...
        // 添加名称为 appsettings.json 和 appsettings.{环境名称}.json 的文件配置源
        // 全部以可选形式添加，可以不存在
        ...
        // 从宿主配置的 "hostBuilder:reloadConfigOnChange" 配置节获取值用来确定是否需要打开针对文件变动的监控
...
#pragma warning restore CS0618
{
    // 这三个 CancellationTokenSource 用来触发应用的三种状态（启动完成、停止开始时、停止完成）
    // 并通过颁发的 CancellationToken 注册回调，在这三种状态触发后执行回调
    ...
    // 这三个属性用来通过对应的 CancellationTokenSource 获取 CancellationToken
...
public partial class ConsoleLifetime : IHostLifetime, IDisposable
{
    ...
    // 完成对 IHostApplicationLifetime.ApplicationStarted 与 IHostApplicationLifetime.ApplicationStopping 的回调注册
    // 并注册对 "Ctrl + C" 快捷键的处理
    public Task WaitForStartAsync(CancellationToken cancellationToken)
    {
        // 根据 ConsoleLifetimeOptions.SuppressStatusMessages 的值决定是否通过注册回调，输出应用状态改变日志
    ...
    // 注册对 "Ctrl + C" 快捷键处理
    // 快捷键被按下后调用 IHostApplicationLifetime.StopApplication 方法触发回调
    ...
    // ConsoleLifetime 对 IHostLifetime.StopAsync 方法实现不做任何事情
    // 即不支持通过 ConsoleLifetime 停止宿主
...
internal sealed class Host : IHost, IAsyncDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        using (var cts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, _applicationLifetime.ApplicationStopping))
        {
            ...
            // 筛选出实现了 IHostedLifecycleService 接口的服务
            _hostedLifecycleServices = GetHostLifecycles(_hostedServices);
            // 标记启动中
            _hostStarting = true;

            // 根据 HostOptions.ServicesStartConcurrently 的值设置所有服务是否并发启动
            bool concurrent = _options.ServicesStartConcurrently;
            // 不是并发启动服务则在遇到第一个服务启动发生错误时抛出异常
            ...
            await ForeachService(_hostedServices, cancellationToken, concurrent, abortOnFirstException, exceptions,
                async (service, token) =>
                {
                    ...
                    {
                        // 如果是继承 BackgroundService 类型的服务，需要验证 BackgroundService._executeTask 是否为空
    ...
    {
        // 继承 BackgroundService 的类可能在重写 StartAsync 方法时没有在内部调用 base.StartAsync
        // 造成 BackgroundService._executeTask 为空的情况
    ...
    // 遍历 IHostedService 或 IHostedLifecycleService 并通过指定的委托调用他们定义的某个以 Start 或 Stop 为前缀的方法
        ...
        if (concurrent) // 并发启动
        {
            ...
            if (tasks is not null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    // 会从 Task.Exception 这个异常中取出 AggregateException.InnerExceptions 的第一个异常抛出
...
public static class HostingAbstractionsHostExtensions
{
    ...
    // 异步方式停止宿主
    // 支持通过设定停止等待超时时间，在所有服务停止完成前提前结束 StopAsync 异步方法
    ...
    // 同步方式启动宿主
    // 这是为长时间运行的服务准备的启动方法
    // 阻塞主线程直到调用 IHostApplicationLifetime.StopApplication 方法触发回调
    ...
    public static async Task WaitForShutdownAsync(this IHost host, CancellationToken token = default)
    {
        ...
        // 注册回调，支持通过外部 CancellationTokenSource 停止宿主
        ...
        // 注册回调，通过调用 IHostApplicationLifetime.StopApplication 方法触发回调
