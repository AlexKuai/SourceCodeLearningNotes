...
public class ServiceDescriptor
{
    ...
    // 不同生命周期的 ServiceDescriptor 创建方法，内部大部分最终都是调用 Describe 静态方法创建 ServiceDescriptor
...
public static class ServiceCollectionDescriptorExtensions
{
    ...
    public static void TryAddEnumerable(this IServiceCollection services, ...)
    {
        ...
        // 实现类型是 object 类型或者服务注册类型和实现类型相同都会抛出 ArgumentException
        // 1. new ServiceCollection().TryAddEnumerable(ServiceDescriptor.Singleton<IFoo>(_ => new Foo()));
        //    这种方式最终编译器会生成 IL 指令创建 Func<IServiceProvider, IFoo> 类型的委托
        //    服务注册类型和实现类型相同，抛出 ArgumentException
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // 如果已经存在服务注册类型和实现类型的 ServiceDescriptor，则忽略
...
- ServiceCollectionServiceExtensions  
```C#
// 通过服务注册类型、实现类型、实现工厂、实例注册服务的扩展方法
// 内部最终调用 ICollection<T>.Add 方法添加 ServiceDescriptor
...
public sealed class ServiceProvider : IServiceProvider, IDisposable, IAsyncDisposable
{
    ...
    // 如果 _realizedServices 没有对应服务注册类型的 Func<ServiceProviderEngineScope, object?>> 委托就通过 _createServiceAccessor 创建
    ...
    // 服务实例的解析最终利用服务类型对应的 Func<ServiceProviderEngineScope, object?> 委托获取
    ...
    internal ServiceProvider(ICollection<ServiceDescriptor> serviceDescriptors, ServiceProviderOptions options)
    {
        ...
        // 使用注册的 ServiceDescriptor 创建 ServiceCallSite 工厂
    ...
    private void ValidateService(ServiceDescriptor descriptor)
    {
        ...
        try
        {
            ...
            // 在得到参数类型对应的 ServiceCallSite 时用来判断是否在解析链中存在循环依赖
    ...
    private Func<ServiceProviderEngineScope, object?> CreateServiceAccessor(Type serviceType)
    {
        ...
        if (callSite != null)
        {
            ...
            if (callSite.Cache.Location == CallSiteResultCacheLocation.Root)
            {
                ...
                // 下次解析对应的服务时直接返回（为注册 Singleton 生命周期服务提供解析优化，提高性能）
                return scope => value;
            }
...
internal sealed class ServiceProviderEngineScope : IServiceScope, IServiceProvider, IAsyncDisposable, IServiceScopeFactory
{
    ...
    // 由当前容器提供的服务，并且实现了 IDisposable 或 IAsyncDisposable 接口，则会保存在此集合中
    ...
    // 由当前容器提供的服务，并且生命周期的是 Singleton 或 Scoped 的，则会保存在此字典中
    ...
    // 实际的服务都是通过 ServiceProvider._engine 来解析的
    ...
    // 支持异步方式执行释放动作
    // 实现 IAsyncDisposable 接口的服务，如果执行 DisposeAsync 方法非同步完成，则通过异步方式执行
...
internal sealed class CallSiteFactory : IServiceProviderIsService
{
    ...
    // ServiceCallSite 缓存表
    // ServiceCacheKey 的唯一性由内部的服务类型和槽位置决定
    ...
    private void Populate()
    {
        foreach (ServiceDescriptor descriptor in _descriptors)
        {
            ...
            if (serviceType.IsGenericTypeDefinition)
            {
                ...
                // 服务注册类型是开放泛型类型时必须通过实现类型并且实现类型也必须是开放泛型类型，否则抛出 ArgumentException
                ...
                // 实现类型不能是抽象类型或接口等开放类型（无法实例化的），否则抛出 ArgumentException
                ...
                // 泛型参数的数量必须匹配，否则抛出 ArgumentException
            ...
            {
                // 服务注册类型是封闭类型并且实现实例和实现工厂为 null 时必须具有实现类型
                Debug.Assert(descriptor.ImplementationType != null);
                Type implementationType = descriptor.ImplementationType;

                // 实现类型不能是开放泛型类型、抽象类型、接口，否则抛出 ArgumentException
            ...
            // ServiceDescriptorCacheItem.Last 指向最后添加的服务注册类型
            // ServiceDescriptorCacheItem._item 指向最早添加的服务注册类型
            // ServiceDescriptorCacheItem._items 保存除 ServiceDescriptorCacheItem._item 以外的注册链
    ...
    internal ServiceCallSite? GetCallSite(ServiceDescriptor serviceDescriptor, CallSiteChain callSiteChain)
    {
        if (_descriptorLookup.TryGetValue(serviceDescriptor.ServiceType, out ServiceDescriptorCacheItem descriptor))
        {
            ...
            // 因为需要同步对 _callSiteCache 字典添加创建的 ServiceCallSite 作为缓存，这时候如果服务注册类型是开放类型，那作为字典的 Key 是没有意义的
    ...
    private ServiceCallSite? CreateCallSite(Type serviceType, CallSiteChain callSiteChain)
    {
        ...
        // C -> D -> A
        // E -> D -> A
        // 在并发解析 C、E 的过程中需要保证他们依赖的 D、A 是同一个 ServiceCallSite 实例
        lock (callsiteLock)
        {
            // 检查循环依赖
    ...
    {
        // 根据服务注册类型查找表得到 ServiceDescriptor 注册链 ServiceDescriptorCacheItem
        if (_descriptorLookup.TryGetValue(serviceType, out ServiceDescriptorCacheItem descriptor))
        {
            // 使用最后注册的 ServiceDescriptor 对应默认槽 0
    ...
    private ServiceCallSite? TryCreateExact(ServiceDescriptor descriptor, Type serviceType, CallSiteChain callSiteChain, int slot)
    {
        ...
        if (serviceType == descriptor.ServiceType)
        {
            ...
            if (descriptor.ImplementationInstance != null)
            {
                ...
                // 创建 ConstantCallSite 不需要 ResultCache，因为常量对象已经由 ServiceDescriptor.ImplementationInstance 引用
            ...
            else if (descriptor.ImplementationType != null)
            {
                ...
                // 创建 ConstructorCallSite 时需要传入 callSiteChain 验证循环依赖
            ...
            CallSiteChain callSiteChain)
    {
        try
        {
            ...
            // 得到实现类型公开实例构造函数的定义元数据（BindingFlags.Public | BindingFlags.Instance）
            ...
            else if (constructors.Length == 1)
            {
                ...
                // 只要存在无法解析的参数类型，则抛出异常（throwIfCallSiteNotFound: true）
            ...
            // 3. 所有候选构造函数的参数列表必须是最优构造函数的参数列表的子集
            ...
            for (int i = 0; i < constructors.Length; i++)
            {
                ...
                // 根据所有参数类型得到对应的 ServiceCallSite
                // 如果存在无法解析的参数类型时返回 null（throwIfCallSiteNotFound: false）
                ...
                {
                    // 因为数组元素已经降序排列（最长参数列表），首次参数解析成功的构造函数记录为候选最优构造函数
                    ...
                    else
                    {
                        ...
                        {
                            // 后续的候选构造函数如果存在参数类型不在候选最优构造函数的参数列表中（不满足子集原则），则抛出 InvalidOperationException
            ...
            else
            {
                ...
                // 要么属于候选最优构造函数，要门不满足子集原则，抛出 InvalidOperationException
            ...
            bool throwIfCallSiteNotFound)
    {
        ...
        for (int index = 0; index < parameters.Length; index++)
        {
            ...
            if (callSite == null)
            {
                // 没有得到 ServiceCallSite 时是否启用异常抛出
                ...
                // 只要存在参数类型无法得到 ServiceCallSite 时就返回 null
                return null;
            }
    ...
    {
        // 服务类型是被构造的泛型类型（封闭类型）并且在服务注册类型查找表中存在泛型类型定义（开放泛型类型）的服务注册类型
    ...
    private ServiceCallSite? TryCreateOpenGeneric(ServiceDescriptor descriptor, Type serviceType, CallSiteChain callSiteChain, int slot, bool throwOnConstraintViolation)
    {
            ...
            serviceType.GetGenericTypeDefinition() == descriptor.ServiceType)
        {
            ...
            try
            {
                // 根据服务类型的泛型实参构建实现类型（开放泛型类型）的封闭类型
            ...
            // 开放泛型类型的服务注册，在解析封闭服务类型创建 ServiceCallSite 时只能是 ConstructorCallSite
            // 因为需要利用服务类型的泛型实参构建封闭实现类型，所以在服务注册时只能通过 ImplementationType 指定实现类型（通过构造函数创建实例）
    ...
    private ServiceCallSite? TryCreateEnumerable(Type serviceType, CallSiteChain callSiteChain)
    {
        ...
        try
        {
                ...
                serviceType.GetGenericTypeDefinition() == typeof(IEnumerable<>))
            {
                ...
                // 如果泛型实参的类型是非泛型类型
                // 直接得到 ServiceDescriptorCacheItem 遍历 ServiceDescriptor
                    ...
                    _descriptorLookup.TryGetValue(itemType, out ServiceDescriptorCacheItem descriptors))
                {
                    for (int i = 0; i < descriptors.Count; i++)
                    {
                        ...
                        // 因为是非泛型类型，所以直接调用 TryCreateExact
                        ServiceCallSite? callSite = TryCreateExact(descriptor, itemType, callSiteChain, slot);
                        Debug.Assert(callSite != null);

                        // 得到基础类型值最大的缓存位置（CallSiteResultCacheLocation）
                        // 基础类型值从小到达依次为 Root -> Scope -> Dispose -> None
                ...
                {
                    // 如果是被构造的泛型类型（封闭类型）
                    // 不能直接通过服务注册类型查找表得到 ServiceDescriptorCacheItem，因为服务注册可能是通过开放泛型类型注册的
                    int slot = 0;
                    // 遍历所有 ServiceDescriptor
                    for (int i = _descriptors.Length - 1; i >= 0; i--)
                    {
                        ServiceDescriptor descriptor = _descriptors[i];
                        // 被构造的泛型类型（封闭类型）可能通过 TryCreateExact 和 TryCreateOpenGeneric 得到 ServiceCallSite
                        // TryCreateExact 会通过 descriptor.ServiceType == itemType 过滤
                        // TryCreateOpenGeneric 会通过 
                    ...
                    // 反转列表，通过 IEnumerable<> 方式得到服务，需要按服务注册时的顺序从前往后排列
                    callSites.Reverse();
                }
    ...
    private struct ServiceDescriptorCacheItem
    {
        ...
        {
            // 每次添加都初始化新的 ServiceDescriptorCacheItem，保证其是不可变的
...
internal abstract class ServiceCallSite
{
    ...
    // 保存如下服务实例：
    // 1. 通过 ConstantCallSite 保存常量值
    // 2. 通过 ConstantCallSite 保存 ServiceDescriptor.ImplementationInstance
    // 3. 生命周期为 Singleton 的服务
    public object? Value { get; set; }

    // 捕获实现了 IDisposable 或 IAsyncDisposable 的服务
    public bool CaptureDisposable =>
        ImplementationType == null ||
        typeof( ).IsAssignableFrom(ImplementationType) ||
...
// 实现类型：
// 1. CallSiteValidator 使用 ServiceProviderOptions 选项中的 ValidateOnBuild 和 ValidateScopes 配置验证服务有效性和范围有效性
// 2. CallSiteRuntimeResolver 使用反射方式解析服务
// 3. ILEmitResolverBuilder ILEmit 使用方式解析服务
...
internal sealed class CallSiteRuntimeResolver : CallSiteVisitor<RuntimeResolverContext, object?>
{
    ...
    protected override object? VisitRootCache(ServiceCallSite callSite, RuntimeResolverContext context)
    {
        ...
        lock (callSite)
        {
            ...
            // 捕获服务是否需要推入服务范围的 ServiceProviderEngineScope._disposables 集合中
            serviceProviderEngine.CaptureDisposable(resolved);
            // 根服务范围的服务实际上并不保存在 ServiceProviderEngineScope.ResolvedServices 字典中
            // 服务实际保存在 ServiceCallSite 的 Value 字段中
    ...
    {
        // 如果解析服务的范围是根服务范围，换用 VisitRootCache（这时候的 Scope 等价于 Root）
        ...
        RuntimeResolverLock lockType)
    {
        ...
        // 当前解析上下文没有对应的锁定范围，对 ServiceProviderEngineScope.ResolvedServices 加锁
    ...
    {
        // 执行实际的解析方法并尝试捕获服务是否需要推入服务范围的 ServiceProviderEngineScope._disposables 集合中
    ...
    protected override object VisitConstructor(ConstructorCallSite constructorCallSite, RuntimeResolverContext context)
    {
        ...
        else
        {
            ...
            {
                // 根据每个参数对应的 ServiceCallSite 得到服务实例
    ...
    {
        // 使用 IEnumerable<> 方式得到的服务最终都是存放在 T[] 类型数组中（T 代表服务类型）
...
internal struct RuntimeResolverContext
{
    ...
    // 针对 Scope 和 Root 结果缓存服务范围的操作需要同步完成，保证在服务范围内的单例性
...
public static class ServiceProviderServiceExtensions
{
    ...
    public static IEnumerable<T> GetServices<T>(this IServiceProvider provider)
    {
        ...
        // 因为最终调用 GetRequiredService 方式得到必要服务，所以可能返回的是一个空数组
