...
public static class WebHost
{
    ...
    internal static void ConfigureWebDefaults(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((ctx, cb) =>
        {
            ...
            // 作为后续用来使用静态资源
            // 步骤如下：
            // 1. 尝试从从宿主配置中得到 "staticWebAssets" 配置节保存的表示的静态资源所在目录的路径
            // 2. 如果配置节不存在，则使用 IHostEnvironment.ApplicationName 名称的程序集所在的目录下
            //    "{environment.ApplicationName}.staticwebassets.runtime.json" 作为资源文件路径名
            // 3. 读取文件内容并反序列化为 StaticWebAssetManifest 类型对象
            // 4. 遍历反序列化对象的 StaticWebAssetManifest.ContentRoots 表示的多个静态资源的目录并得到对应的 PhysicalFileProvider
...
{
    // 两个创建 WebHostOptions 的情况
    // 1. 第一个调用 GetWebHostBuilderContext 方法时会创建 WebHostOptions 实例
    //    此时的主配置是当前 HostBuilderContext.Configuration 中保存的宿主配置
    // 2. 在执行 HostingStartup 时创建时会创建 WebHostOptions 实例
    //    此时的主配置就是 GenericWebHostBuilder 中的备用配置
...
internal abstract class WebHostBuilderBase : IWebHostBuilder, ISupportsUseDefaultServiceProvider
{
    ...
    public WebHostBuilderBase(IHostBuilder builder, WebHostBuilderOptions options)
    {
        ...
        // 根据 WebHostBuilderOptions.SuppressEnvironmentConfiguration 的值判断是否需要添加前缀为 "ASPNETCORE_" 的环境变量配置源
    ...
    protected WebHostBuilderContext GetWebHostBuilderContext(HostBuilderContext context)
    {
        ...
        if (!context.Properties.TryGetValue(typeof(WebHostBuilderContext), out var contextVal))
        {
            ...
            // 最终 IWebHostEnvironment.WebRootPath 的值就是 "{ContentRootPath}/wwwroot"
...
// 收集配置的先后顺序：
// 1. 通过构造函数收集的
// 2. 通过 ConfigureWebHostDefaults 方法收集的
// 3. 通过 IWebHostBuilder 调用方法收集的
//    会存在通过基于约定的 Startup 收集的（使用 Startup 类型）
// 4. 通过强类型 Startup 收集的（实现 IHostingStartup 接口）
// 5. 通过基于约定的 Startup 收集的（使用 "startupAssembly" 配置节表示的程序集中发现的 Startup 类型）
...
#pragma warning restore CS0618 
            ...
            // 注册 ActivitySource，使用 使用 Func<IServiceProvider, ActivitySource> 工厂
    ...
    private void ScanAssemblyAndRegisterStartup(HostBuilderContext context, IServiceCollection services, WebHostBuilderContext webhostContext, WebHostOptions webHostOptions)
    {
        try
        {
            ...
            // 2. "Startup"
            // 3. 以上名称前添加 "{webHostOptions.StartupAssembly}." 前缀
    ...
    public IWebHostBuilder UseStartup<TStartup>(Func<WebHostBuilderContext, TStartup> startupFactory)
    {
        ...
        // 将定于 startupFactory 委托的原型方法的类型所在的程序集名称作为 "applicationName" 配置节的值
        UseSetting(WebHostDefaults.ApplicationKey, startupAssemblyName);
    ...
    private void UseStartup(Type startupType, HostBuilderContext context, IServiceCollection services, object? instance = null)
    {
        ...
        try
        {
            ...
            // 将 Startup 实例存入由 IHostBuilder.Properties 表示的构建共享字典中
            ...
            // 1. public
            // 2. 返回值类型是 void
            // 3. 方法有且仅有一个参数，参数的类型需要是实现 IContainerBuilder 接口
            // 4. 找到的方法不能存在重载
...
{
    // 通过调用 Func<Action<IApplicationBuilder>, Action<IApplicationBuilder>> 并传入后续的中间件建造者配置委托
...
// 并最终将中间件构建成 RequestDelegate -> RequestDelegate -> ... -> RequestDelegate 的请求处理委托链
...
public partial class ApplicationBuilder : IApplicationBuilder
{
    ...
    // 利用共享字典保存表示根容器的 IServiceProvider
    ...
    // ApplicationServices 和 ServerFeatures 两个属性的存取都是通过这个共享字典完成的
    ...
    public RequestDelegate Build()
    {
        ...
        // 利用末端处理器，从后向前开始遍历并执行 Func<RequestDelegate, RequestDelegate> 构建请求处理委托链
...
public class MiddlewareFactory : IMiddlewareFactory
{
    ...
    // 因为 IMiddlewareFactory 被注册为 Scoped 生命周期，并且在请求处理阶段由 HttpContext.RequestServices 表示的范围容器创建
...
public static class UseMiddlewareExtensions
{
        ...
        params object?[] args)
    {
        ...
        // 2. 不能定义 "Invoke" 或 "InvokeAsync" 方法的重载
        // 3. "Invoke" 或 "InvokeAsync" 方法只能存在一个
        // 4. "Invoke" 或 "InvokeAsync" 方法返回类型必须是 Task
        // 5. "Invoke" 或 "InvokeAsync" 方法必须存在参数，并且第一个参数是 HttpContext 类型
    ...
    private sealed class ReflectionMiddlewareBinder
    {
        ...
        public RequestDelegate CreateMiddleware(RequestDelegate next)
        {
            ...
            // 直接利用 "Invoke" 或 "InvokeAsync" 方法创建 RequestDelegate 委托返回
            if (_parameters.Length == 1)
            {
                return (RequestDelegate)_invokeMethod.CreateDelegate(typeof(RequestDelegate), instance);
            }
            
            // 如果 "Invoke" 或 "InvokeAsync" 方法有除 HttpContext 类型参数外的其他参数
            // 则需要检查是否支持运行时动态代码编译
            // 如果是则使用构建动态代码后编译表达式成 Func<object, HttpContext, IServiceProvider, Task> 委托执行
            ...
            // 但 "Invoke" 或 "InvokeAsync" 方法的参数可以由 HttpContext.RequestServices 表示的范围容器创建注入
            return context =>
            {
    ...
    private static Func<T, HttpContext, IServiceProvider, Task> CompileExpression<T>(MethodInfo methodInfo, ParameterInfo[] parameters)
    {
        ...
        // 分别是：
        // 1. 中间件实例，参数名 "middleware"，object 类型
        // 2. 请求上下文，参数名 "httpContext"，HttpContext 类型
        // 3. 范围容器，参数名 "serviceProvider"，IServiceProvider 类型
...
{
    // 根据不同服务器提供的 IFeatureCollection 特性集合创建 HttpContext
...
internal sealed partial class GenericWebHostService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (addresses != null && !addresses.IsReadOnly && addresses.Count == 0)
        {
            ...
            // 从应用配置读取 "http_ports" 和 "https_ports" 配置节表示的端口
