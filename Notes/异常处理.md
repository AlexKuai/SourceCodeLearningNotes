...
sequenceDiagram
    ...
    participant EH as ExceptionHandlerMiddleware<br/>(或 DeveloperExceptionPage)
    participant Pipeline as 后续中间件 + 业务
    participant Handler as 异常 / 状态码处理器

    C->>SCP: HTTP 请求
    SCP->>EH: next(ctx)
    EH->>Pipeline: try { await next(ctx); }
...
flowchart TD
    ...
    Catch --> Aborted{ex 是 OperationCanceled/IO<br/>且 RequestAborted 已请求取消?}
...
flowchart TD
    ...
    CheckEn -->|true| Recheck{shouldCheckEndpointAgain<br/>且现在 endpoint 有 ISkip 元数据?}

    Recheck -->|是| Return
    Recheck -->|否| Conditions{HasStarted/StatusCode<br/>不在 400-599/ContentLength<br/>已设/ContentType 已设?}
