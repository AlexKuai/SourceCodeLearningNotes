...
sequenceDiagram
    ...
    Note over SP: UseServiceProviderFactory to CreateBuilder + 用户 ConfigureContainer 委托
    Note over SP: CreateServiceProvider

    HB->>U: 6. ResolveHost[IHost]
```
...
```mermaid
flowchart LR
    subgraph HCfg [宿主配置 hostConfiguration]
        H1["内存源(空)"]
        H2["DOTNET_ 环境变量"]
        H3["命令行参数"]
        H1 & H2 & H3 --> HostCfg["IConfiguration (hostConfiguration)"]
    end

    subgraph ACfg [应用配置 appConfiguration]
...
flowchart TD
    ...
    Result -->|正常完成| End1[返回]
    Result -->|OperationCanceled 且 _stopCalled| End2["返回(预期取消)"]
