...
flowchart TB
    subgraph Legacy ["传统：Startup 类"]
        ...
        Startup["class Startup<br/>{ ConfigureServices; Configure; }"]
        L3 -. 反射 .-> Startup
    end

    subgraph Generic ["通用宿主：lambda Configure"]
        G1["Host.CreateDefaultBuilder()"]
        G2[".ConfigureWebHostDefaults(web => {<br/>  web.ConfigureServices(...);<br/>  web.Configure(app => ...);<br/>})"]
        G3[".Build().Run()"]
        G1 --> G2 --> G3
    end

    subgraph Minimal ["Minimal API：扁平化"]
        M1["var builder = WebApplication.CreateBuilder(args);"]
        M2["builder.Services.Add...();"]
        M3["var app = builder.Build();"]
        M4["app.UseXxx();<br/>app.MapGet(...);"]
        M5["app.Run();"]
        M1 --> M2 --> M3 --> M4 --> M5
    end
```
